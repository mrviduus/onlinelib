...
- **No publish step / no secret in CI.** The owner publishes manually with their nuget.org API key: `dotnet pack backend/src/Ai/TextStack.Ai.Mcp -c Release` then `dotnet nuget push backend/src/Ai/TextStack.Ai.Mcp/bin/Release/TextStack.Mcp.1.0.0.nupkg --api-key <KEY> --source https://api.nuget.org/v3/index.json`.
