...
### Reader — PDF reading progress is page-based (library % + resume) — web + backend (ADR-012 S2)
...
- **Senior fix round (post-QA).** (1) **Eval relevance gate hardened** — recall@k alone was gameable (an agent that floods the `MaxRecommendations:8` cap with real-but-irrelevant catalog books that include the goldens scored recall 1.0 while the hallucination check only probes slug-existence). Added **precision@k** (relevant-returned / total-library-returned) and **F1** to `LibrarianEvalResult` + per-case + the admin endpoint; a flood-the-cap agent now scores precision 0.125 / F1 ≈ 0.22 at recall 1.0. (2) **External recs re-projected, not model-trusted** — `RetrievedCatalog` now stores the **full harvested Open Library row** (title + authors + year + pages + work key), and an `open_library` recommendation re-projects all of those from the matched row instead of trusting the model, so it can't attach a fabricated author/year to a real OL title; a field the OL row lacked stays null. (3) **External-title matching normalized identically on both sides** — one `NormalizeTitleKey` (sanitize → NFC → trim → collapse internal whitespace → lowercase) keys the stored set and the model echo, so a legitimate rec is no longer silently dropped over case/whitespace/Unicode skew. (4) **Endpoint min-query guard** — `POST /me/librarian` rejects a <2-char query with 400 before running the agent (mirrors the RAG/hybrid-search ≥2-char guard). Library anti-hallucination invariant unchanged. UnitTests green (947) incl. flood-resistance + external re-projection + normalized-matching + min-query.

### Enrichment Agent — book metadata via a ReAct tool-use loop (AI-Agent-1) — backend (2026-06-23)

The first true **agent**: book metadata (genre / publish year / description) for **user-uploaded books** now comes from a ReAct loop instead of one Ollama guess. The `EnrichmentAgent` runs on the existing `AgentLoop` runtime (`TextStack.Ai.Agents` — plan→act→observe with hard `MaxSteps`/`CostCapUsd` caps + persisted transcript), reasons over title+author+a sanitized excerpt, and **decides to call tools** when a field is missing or low-confidence: two new keyless `ITool`s, `search_open_library` (title/author → author/year/subjects/pages/work-key) and `get_open_library_work` (key → description), against the free Open Library API. It **cross-checks and reconciles** (prefers the earliest *corroborated* publish year, source `reconciled`), commits a field only at **confidence ≥ 0.7** (`Enrichment:ConfidenceThreshold`, configurable), and on budget/uncertainty emits a calibrated **`unknown`** (null) rather than a hallucinated value — overall confidence = min over *committed* fields (an abstained field doesn't tank a confident one). Wired **fire-and-forget for user books** from `UserIngestionService` post-extraction → `IBookMetadataGenerator.EnrichAsync`; writes only NULL/empty fields, honors the `manual` source guard, and persists **per-field provenance** (`user_books.metadata_provenance_json`) + **confidence** (`user_books.metadata_confidence`). Telemetry: `agent_run` gains `confidence` + `tool_calls_count`. External text is **untrusted**: both tools' free-text (and the user excerpt) run through `ExternalTextSanitizer`, length-capped, and enter the model strictly as DATA. Safe degradation: on a keyless host the agent fails the first model call and **falls back to the legacy Ollama** path (now logged once at startup via `EnrichmentKeyCheck` so the no-op is visible, not silent). **Eval**: `EnrichmentEvalRunner` (30 goldens: canonical / ambiguous / unknown bands) scores genre/year accuracy plus the headline **calibration** (committed ⇒ correct) and **honest-unknown rate** (abstention on the unknown band), with indeterminate ground-truth (e.g. BC dates) opting out of year scoring so a correct confident answer isn't penalized; admin-runnable via `POST /admin/ai/enrichment/eval` (503 keyless). Model route `bookmeta.agent → openai-explain` (gpt-4.1-mini). `dotnet build` green, `dotnet format` clean; 912 unit + 65 AiEvals tests green (tool parse/no-match/timeout/object-vs-string description, reconcile/confidence/stopping, excerpt-injection sanitized, budget-exhausted tool-count, calibration scoring). **Deferred**: Google Books tool, web search, catalog `Edition` enrichment, admin-configurable threshold + low-confidence review UI. Design: `docs/04-dev/agents-roadmap.md`. **Needs a real-model run** to read the live calibration number on gpt-4.1-mini (hit the admin eval endpoint with a key).

### Ask this book — mobile reader: user-books + on-demand index gating (AI-027 P1/P2) — mobile (2026-06-22)

"Ask this book" now actually works in the **mobile** reader, and for the **first time on user-uploaded books** (the priority case — users upload books to read *and* ask about). Two defects fixed: (1) user books had **no Ask button** — the button was gated on `explainBookId`, which only the catalog source set; (2) **no on-demand index trigger**, so a fresh/0-chunk book returned "insufficient" and looked broken. The reader now builds a single **`AskTarget`** (`{ kind: 'edition' | 'userbook', id }`) from whichever source loaded (catalog `useEditionReaderSource` / user-book `useUserBookReaderSource`), threads it through `ReaderRuntime` → `ReaderShell`, and gates the Ask button (`showAsk`) on having a target for **both** kinds. `AskSheet` is generalized from a raw `editionId` to the target and selects the endpoint family by `kind` — catalog `GET/POST /books/{id}/index` + `POST /books/{id}/ask`, user-book `GET/POST /me/books/{id}/index` + `POST /me/books/{id}/ask` (Bearer). **On-demand index gate**: on open it GETs index status; if `NotIndexed`/`Failed` it POSTs prepare and **polls every 3s (5 min cap)** showing "Indexing this book…", enabling the composer only at `Ready`; `Failed`/timeout shows a clear error + Retry, a `/ask` 503 surfaces a "not available" notice, 401 keeps the sign-in CTA, and `insufficient` answers render their hint. JSON one-shot only — **SSE streaming + multi-turn history are out of this slice** (web keeps its streaming path). Citation-tap jump-to-passage unchanged; Explain wiring (also reads `explainBookId`) untouched. Shared `@textstack/shared` gains `RagIndexState`/`RagIndexStatus`/`AskTarget` types and `rag.ts` funcs (`askUserBook`, `getIndexStatus`/`prepareIndex`, `getUserIndexStatus`/`prepareUserIndex`); the catalog `ask` signature is unchanged (web consumes only shared *types*, not `ragApi`). Mobile `tsc --noEmit` clean; shared 149 tests green. Backend (P1/P2 endpoints) already shipped. **Needs on-device verification**: user-book ask end-to-end + first-index prepare latency.
