...
test.describe('QA-002: Multi-Language Library Navigation', () => {
  test('UK book from EN library links to /uk/ (book language)', async ({ authedPage: page }) => {
    ...
    // Find the UK book in library — links should use book language /uk/
  ...
  test('EN book from UK library links to /en/ (book language)', async ({ authedPage: page }) => {
    ...
    // Find the EN book in library — links should use book language /en/
    const bookLinks = page.locator('.library-list-item__title a, .library-card__title a, .library-list-item__cover, .library-card__cover')
    const count = await bookLinks.count()
  ...
  test('clicking UK book from EN library loads reader without error', async ({ authedPage: page }) => {
    const { ukBook } = getTestData()
    if (!ukBook) {
      test.skip()
      return
    ...
    // Read UK book briefly to add to library
    await page.goto(`/uk/books/${ukBook.slug}/${ukBook.firstChapterSlug}`)
    await page.waitForTimeout(4000)
    ...
    if (await bookItem.isVisible()) {
      ...
      // Should navigate to /uk/ (book language)
      expect(page.url()).toContain('/uk/')
      // Should not show error
      await expect(page.locator('text=Error loading chapter')).not.toBeVisible()
      await expect(page.locator('text=Chapter not found')).not.toBeVisible()
    }
  })
})
