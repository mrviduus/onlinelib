...
public sealed class RagEvalRunner(ILogger<RagEvalRunner> logger)
{
        ...
        CancellationToken ct)
    {
        ...
        foreach (var g in retrievalGoldens)
        {
            ...
            var chunks = await rag.RetrieveAsync(editionId, g.Question, k, maxChapterOrd: null, includeSummaries: false, ct);
        ...
        {
            ct.ThrowIfCancellationRequested();
            var chunks = await rag.RetrieveAsync(editionId, g.Question, k, maxChapterOrd: g.GateChapterOrd, includeSummaries: false, ct);
