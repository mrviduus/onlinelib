...
import { composeQuotedQuestion, parseQuotedContent } from './chatQuote'

describe('chatQuote codec', () => {
  ...
  it('preserves a blank line inside the passage', () => {
    const passage = 'first\n\nthird'
    const content = composeQuotedQuestion(passage, 'why?')
    expect(content).toBe('> first\n>\n> third\n\nwhy?')
    expect(parseQuotedContent(content)).toEqual({ quote: passage, text: 'why?' })
  })
